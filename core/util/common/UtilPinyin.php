...
class UtilPinyin
{
    /**
     * 将中文转换成拼音
     * @param string $_String 需转换的字符串
     * @param mixed $_Code 字符集编码
     * @param mixed $isRemainOtherChars 是否保留其他字符如a-z,0-9,-等等
     */
    public static function translate($_String, $_Code='UTF8', $isRemainOtherChars = true)
    {
     ...
     * 通过中文汉字获取拼音名称
     * @cname string 中文字符串
     * @$outputType int 需要输出的类型
     * 1: 每个字拼音头字母大写
     * 获取第一个字全部拼音及之后每个字母的首字母
     * 2: 除头字母大写外，其它字头字母小写
     * 3: 除头字母大写外，其它字头字母大写
     */
    public static function getPinyinName($cname, $outputType = 2)
    {
        ...
        for ($i = 0; $i < strlen($cname); $i++) {
            $cstr = mb_substr($cname, $i, 1);
            $estr = self::translate($cstr);
            if ($i === 0) {
              $result .= ucfirst($estr);
            } else {
              switch ($outputType) {
...
//测试
//echo UtilPinyin::translate('测试','utf8'); //第二个参数“1”可随意设置即为utf8编码
?>
