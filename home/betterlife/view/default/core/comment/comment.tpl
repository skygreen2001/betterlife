    ...
    <div id="content" class="contentBox">
        ...
        {if !isset($smarty.get.comment_id)}
            ...
            <div>
                ...
                <b>
                {if $comment.canEdit}[<my:a href="{$url_base}index.php?go={$appName}.comment.comment&comment_id={$comment.comment_id}&blog_id={$comment.blog_id}&pageNo={$smarty.get.pageNo|default:"1"}">改</my:a>]{/if}
                {if $comment.canDelete}[<my:a href="{$url_base}index.php?go={$appName}.comment.delete&comment_id={$comment.comment_id}&blog_id={$comment.blog_id}&pageNo={$smarty.get.pageNo|default:"1"}">删</my:a>]{/if}
                </b>
            </div>
            {/foreach}
        {/if}
        ...
        <div>
            ...
            <form name="commentForm" method="post">
                ...
                <input type="submit" value="提交" class="btnSubmit" /> <input class="btnSubmit" onclick="location.href='{$url_base}index.php?go={$appName}.comment.comment&blog_id={$smarty.get.blog_id}&pageNo={$smarty.get.pageNo|default:"1"}'" type="button" value="返回" />
            </form>
        </div>
