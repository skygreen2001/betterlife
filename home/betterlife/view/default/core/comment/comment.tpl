...
{block name=body}
    ...
    <div id="content" class="contentBox">
        <my:a href='{$url_base}?go={$app_name}.auth.logout'><b>退出</b></my:a><br/>
        <my:a href='{$url_base}?go={$app_name}.blog.display&pageNo={$smarty.get.pageNo|default:"1"}'><b>博客列表</b></my:a>
        ...
        {if !isset($smarty.get.comment_id)}
            ...
            <div>
                ...
                <b>
                {if $comment.canEdit}[<my:a href="{$url_base}index.php?go={$app_name}.comment.comment&comment_id={$comment.comment_id}&blog_id={$comment.blog_id}&pageNo={$smarty.get.pageNo|default:"1"}">改</my:a>]{/if}
                {if $comment.canDelete}[<my:a href="{$url_base}index.php?go={$app_name}.comment.delete&comment_id={$comment.comment_id}&blog_id={$comment.blog_id}&pageNo={$smarty.get.pageNo|default:"1"}">删</my:a>]{/if}
                </b>
            </div>
            {/foreach}
        {/if}
        ...
        <div>
            ...
            <form name="commentForm" method="post">
                ...
                <input type="submit" value="提交" class="btnSubmit" /> <input class="btnSubmit" onclick="location.href='{$url_base}index.php?go={$app_name}.comment.comment&blog_id={$smarty.get.blog_id}&pageNo={$smarty.get.pageNo|default:"1"}'" type="button" value="返回" />
            </form>
        </div>
